            ...
            try:
                ...
                **[지시사항: HTML <body> 내용만 작성. <html>, <head> 및 불필요한 마크다운 기호(```)는 사용하지 마세요]**
                ...
                <div class="answer-sheet">
                    ...
                    {f"<h4>유형 1. 어휘 문제 정답 및 풀이 ({current_count_t1}문항)</h4><br>[지시]: {current_count_t1}문항의 정답과 뜻풀이를 모두 작성. 각 문제의 해설은 줄 바꿈(<br>)하여 구분할 것.<br><br>" if current_count_t1 > 0 else ""}
                    ...
                    {f"<h4>유형 5. 소설 속 상황 요약 모범 답안</h4><br>[지시]: 유형 5의 질문에 대한 모범적인 분석 내용을 작성하여 제시.<br><br>" if select_t5 else ""}

                    {f"<h4>유형 6. 인물 관계도 및 갈등 모범 답안</h4><br>[지시]: 유형 6의 질문에 대한 모범적인 분석 내용을 작성하여 제시.<br><br>" if select_t6 else ""}

                    {f"<h4>유형 7. 핵심 갈등 구조 및 심리 모범 답안</h4><br>[지시]: 유형 7의 질문에 대한 모범적인 분석 내용을 작성하여 제시.<br><br>" if select_t7 else ""}

                    {f"<h4>유형 8. {current_title_t8} 모범 답안 ({current_count_t8}문항)</h4><br>[지시]: 유형 8({current_title_t8})의 모범 답안을 상세하게 작성. 각 문제의 모범 답안이 끝날 때마다 **<br><br><br> 태그를 사용하여 충분히 간격을 확보하여 분리할 것.**<br><br>" if current_count_t8 > 0 else ""}
