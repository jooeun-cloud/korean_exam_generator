...
def non_fiction_app():
    ...
    with st.sidebar:
        ...
        if current_d_mode == 'AI 생성':
            ...
            topic_input = "사용자 지문"; domain_select = "사용자 입력"; difficulty = "사용자 지정"
        ...
        s2 = st.checkbox("2. 내용 일치 O/X", key="select_t2"); c2 = st.number_input(" - 문항 수", 1, 10, 2, key="t2") if s2 else 0
    ...
    if st.session_state.generation_requested:
        ...
        if not topic_input and current_d_mode == 'AI 생성': st.warning("주제를 입력하세요."); st.session_state.generation_requested = False; return
        status = st.empty()
        try:
            reqs = []
            if s1: reqs.append('<div class="question-box"><span class="question-text">1. ' + label_t1 + '</span><div class="write-box"></div></div>')
            if s2: reqs.append('<h3>내용 일치 O/X (' + str(c2) + '문항)</h3>- 문항 끝에 ( O / X ) 포함.')
            if s3: reqs.append('<h3>빈칸 채우기 (' + str(c3) + '문항)</h3>- 빈칸은 `<span class="blank">&nbsp;&nbsp;&nbsp;&nbsp;</span>` 사용. 영어 정답 금지.')
            if s4: reqs.append('<h3>변형 문장 정오판단 (' + str(c4) + '문항)</h3>- 문항 끝에 ( O / X ) 포함.')
            ...
            if s5: reqs.append('<h3>객관식: 세부 내용 파악 (' + str(c5) + '문항)</h3>' + mcq_tpl)
            if s6: reqs.append('<h3>객관식: 추론 및 비판 (' + str(c6) + '문항)</h3>' + mcq_tpl)
            if s7: reqs.append('<h3>객관식: [보기] 적용 문제 (' + str(c7) + '문항) [3점]</h3><div class="question-box"><span class="question-text">[문제번호] 윗글을 바탕으로 [보기]를 이해한 내용으로 적절하지 않은 것은? [3점]</span><div class="example-box">(보기 내용)</div><div class="choices"><div>①...</div><div>②...</div><div>③...</div><div>④...</div><div>⑤...</div></div></div>')
            ...
            # [Step 1] 지문 및 문제지 생성 (SyntaxError 해결을 위해 백슬래시 연산 변수화)
            p1_prompt = """
...
{REQS}
            """.format(
                P_INST = f"**[Step 1] 지문 작성** - 주제: {topic_input}, 난이도: {difficulty}, 길이: 1800자 내외 \n{sum_inst}" if current_d_mode == 'AI 생성' else "**[Step 1] 지문 인식** - 사용자 입력 지문 기반.",
            ...
            for i in range(0, total_q_cnt, BATCH_SIZE):
                ...
                c_sum_p = "- **[필수 - 최우선 작성]**: 답변 맨 위에 `<div class='summary-ans-box'>`를 열고 문단별 요약 예시 답안을 작성하시오." if use_summary and not summary_done else ""
                summary_done = True
                
                p_chunk = """
당신은 대한민국 수능 국어 출제 위원장입니다.
전체 {T_CNT}문제 중, **{SN}번부터 {EN}번까지**의 문제에 대해서만 정답 및 해설을 작성하시오.
[입력된 문제]: {Q_TEXT}

**[지시사항]**
1. HTML 코드만 출력. 절대 제목(`<h1>`, `<h2>`) 생성 금지.
{C_SUM}
2. [해설 규칙]: 객관식은 정답 해설 + 오답 분석 필수. OX/빈칸은 지문 근거 설명 필수.

**[작성 포맷 HTML]**
<div class="ans-item">
    <div class="ans-type-badge">[유형]</div>
    <span class="ans-num">[{SN}] 정답: ...</span>
    <span class="ans-content-title">1. 정답 상세 해설</span><span class="ans-text">...</span>
</div>
                """.format(T_CNT=total_q_cnt, SN=sn, EN=en, Q_TEXT=html_q, C_SUM=c_sum_p)
            ...
            full_html = HTML_HEAD + get_custom_header_html(custom_main_title, topic_input)
            
            if current_d_mode == '직접 입력':
                raw_paras = [p.strip() for p in re.split(r'\n\s*\n', manual_p.strip()) if p.strip()]
                formatted_paras = ""
                for p in raw_paras:
                    formatted_paras += f"<p>{p}</p>"
                    if use_summary: formatted_paras += "<div class='summary-blank'>📝 문단 요약 연습: </div>"
                full_html += f'<div class="passage">{formatted_paras}</div>'
            
            full_html += html_q + html_a + HTML_TAIL
            st.session_state.generated_result = {"full_html": full_html, "main_title": custom_main_title, "topic_title": topic_input}
...
def fiction_app():
    with st.sidebar:
        ...
        uv = st.checkbox("1. 어휘 문제 (단답형)", value=True, key="fic_v"); cv = st.number_input(" - 문항 수", 1, 20, 5, key="f_cv") if uv else 0
        ue = st.checkbox("2. 서술형 심화 (감상/의도)", value=True, key="fic_e"); ce = st.number_input(" - 문항 수", 1, 10, 3, key="f_ce") if ue else 0
        um = st.checkbox("3. 객관식 (일반 5지선다)", value=True, key="fic_m"); cm = st.number_input(" - 문항 수", 1, 10, 3, key="f_cm") if um else 0
        ub = st.checkbox("4. 객관식 (보기 적용 심화)", value=True, key="fic_b"); cb = st.number_input(" - 문항 수", 1, 10, 2, key="f_cb") if ub else 0
    ...
    if st.session_state.generation_requested:
        ...
        status = st.empty(); status.info("⚡ 소설 심층 분석 및 문제 생성 중...")
        try:
            req_list = []
            if uv: req_list.append(f'유형 1. 어휘 문제 ({cv}문항) - 단답형')
            if ue: req_list.append(f'유형 2. 서술형 심화 ({ce}문항) - 의도/효과/이유')
            if um: req_list.append(f'유형 3. 객관식 일반 ({cm}문항) - 수능형 5지선다')
            if ub: req_list.append(f'유형 4. 객관식 보기 ({cb}문항) - 3점 킬러문항')
            if u5: req_list.append('유형 5. 주요 등장인물 정리 (표)')
            if u6: req_list.append('유형 6. 소설 속 상황 요약')
            if u7: req_list.append('유형 7. 인물 관계도 및 갈등 (박스)')
            if u8: req_list.append('유형 8. 갈등 구조 및 심리 정리')
            
            req_str = "\n".join(req_list)
            p1 = """
당신은 수능 문학 출제위원입니다. 작품 '{W_N}'({A_N}) 기반 학생용 문제지(HTML)를 작성하시오.
# 🚨 제목/헤더 생성 금지. 본문 내용부터 출력.

# 🚨 [수능 최고난도 출제 지침]
1. [복합적 사고]: 작품 전체 맥락과 함축적 의미 종합.
2. [매력적인 오답]: 부분적 진실, 주객 전도 함정 배치.
3. [보기 적용]: 비평적 관점을 적용한 3점 문항.

[출제 요청 목록]
{R_STR}
본문:
{BODY}
            """.format(W_N=w_n, A_N=a_n, R_STR=req_str, BODY=text)
            
            res_q = generate_content_with_fallback(p1, status_placeholder=status)
            html_q = res_q.text.replace("```html","").replace("```","").strip()
            
            p2 = """
당신은 수능 문학 해설 위원입니다. 앞서 출제된 문제들에 대한 완벽한 정답 및 해설을 <div class="answer-sheet"> 내부에 작성하시오.
[입력 문제]: {Q_TEXT}
...
def poetry_app():
    with st.sidebar:
        ...
        t1 = st.checkbox("1. 작품 개요 문제", value=True); n1 = st.number_input("수", 1, 5, 1, key="pn1") if t1 else 0
        t2 = st.checkbox("2. 시상 전개 문제", value=True); n2 = st.number_input("수", 1, 5, 1, key="pn2") if t2 else 0
        t3 = st.checkbox("3. 시어 의미 문제", value=True); n3 = st.number_input("수", 1, 5, 2, key="pn3") if t3 else 0
        t4 = st.checkbox("4. 표현 특징 문제", value=True); n4 = st.number_input("수", 1, 5, 2, key="pn4") if t4 else 0
        t5 = st.checkbox("5. 종합 감상 문제", value=True); n5 = st.number_input("수", 1, 5, 1, key="pn5") if t5 else 0
        t6 = st.checkbox("6. 수능 킬러 포인트", value=True); n6 = st.number_input("수", 1, 5, 1, key="pn6") if t6 else 0
        t7 = st.checkbox("7. 연계 비교 문제", value=True); n7 = st.number_input("수", 1, 5, 1, key="pn7") if t7 else 0
        st.header("3️⃣ 추가 세트")
        t8 = st.checkbox("8. OX 세트", value=True); n8 = st.number_input("OX수", 1, 15, 10, key="pn8") if t8 else 0
        t9 = st.checkbox("9. 서술형 세트", value=True); n9 = st.number_input("서술수", 1, 10, 3, key="pn9") if t9 else 0
    ...
    if st.session_state.generation_requested:
        ...
        try:
            r_list = []
            if t1: r_list.append(f"문항1. 작품개요 파악 {n1}개")
            if t2: r_list.append(f"문항2. 시상전개 과정 {n2}개")
            if t3: r_list.append(f"문항3. 시어의 상징성 {n3}개")
            if t4: r_list.append(f"문항4. 표현상 특징/효과 {n4}개")
            if t5: r_list.append(f"문항5. 작품 종합 감상 {n5}개")
            if t6: r_list.append(f"문항6. 수능 필수 개념 {n6}개")
            if t7: r_list.append(f"문항7. 타 작품 연계 비교 {n7}개")
            if t8: r_list.append(f"문항8. OX 정오판단 {n8}개 (정답비공개)")
            if t9: r_list.append(f"문항9. 수능형 서술형 {n9}개")
            
            r_str = "\n".join(r_list)
            p_q = """
당신은 수능 국어 출제 위원장입니다. 현대시 '{W_N}'({A_N}) 기반 학생용 HTML 시험지를 제작하시오.
[지침]: 모든 항목은 '설명'이 아닌 '질문(문항)'이어야 함. 8번 OX는 빈 괄호 ( ) 만 출력. 정답 절대 포함 금지.
본문: {BODY}
요청 문항:
{R_STR}
            """.format(W_N=po_n, A_N=po_a, BODY=text, R_STR=r_str)
            
            res_q = generate_content_with_fallback(p_q, status_placeholder=status)
            html_q = res_q.text.replace("```html","").replace("```","").strip()
            
            p_a = """
당신은 수능 국어 해설 위원입니다. 위 문항들에 대한 완벽한 정답 및 지문 근거 해설을 <div class='answer-sheet'> 내부에 작성하시오.
OX는 반드시 정답(O/X) 표시를 하시오.
[입력 문제]: {Q_TEXT}
            """.format(Q_TEXT=html_q)
...
with col_R:
    if st.session_state.app_mode == "⚡ 비문학 문제 제작":
        ...
        if st.session_state.get("domain_mode_select") == "직접 입력":
            ...
            if m_mode == "단일 지문": st.text_area("지문 입력", height=300, key="manual_passage_input_col_main")
            else:
                ca, cb = st.columns(2)
