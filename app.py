...
def generate_content_with_fallback(prompt, generation_config=None, status_placeholder=None):
    ...
    for model_name in MODEL_PRIORITY:
        try:
            ...
            last_exception = e
            continue 
    raise last_exception if last_exception else Exception("AI 응답 실패")
...
def non_fiction_app():
    ...
    with st.sidebar:
        st.header("🏫 문서 타이틀 설정")
        custom_main_title = st.text_input("메인 타이틀", value="사계국어 모의고사", key="nf_title")
        ...
        cnt_t2 = st.number_input(" - 문항 수", 1, 10, 2, key="t2") if select_t2 else 0
        select_t3 = st.checkbox("3. 빈칸 채우기", key="select_t3")
        cnt_t3 = st.number_input(" - 문항 수", 1, 10, 2, key="t3") if select_t3 else 0
        select_t4 = st.checkbox("4. 변형 문장 정오판단", key="select_t4")
        cnt_t4 = st.number_input(" - 문항 수", 1, 10, 2, key="t4") if select_t4 else 0
        select_t5 = st.checkbox("5. 객관식 (일치/불일치)", value=True, key="select_t5")
        cnt_t5 = st.number_input(" - 문항 수", 1, 10, 2, key="t5") if select_t5 else 0
        select_t6 = st.checkbox("6. 객관식 (추론)", value=True, key="select_t6")
        cnt_t6 = st.number_input(" - 문항 수", 1, 10, 2, key="t6") if select_t6 else 0
        select_t7 = st.checkbox("7. 객관식 (보기 적용 3점)", value=True, key="select_t7")
        cnt_t7 = st.number_input(" - 문항 수", 1, 10, 1, key="t7") if select_t7 else 0
    ...
    if st.session_state.generation_requested:
        ...
        if not topic and current_d_mode == 'AI 생성':
            st.warning("주제 입력 필요"); st.session_state.generation_requested = False; return

        status = st.empty()
        try:
            reqs = []
            if select_t1: reqs.append(f'<div class="question-box"><span class="question-text">1. {label_t1}</span><div class="write-box"></div></div>')
            if select_t2: reqs.append(f'<h3>내용 일치 O/X ({cnt_t2}문항)</h3>')
            if select_t3: reqs.append(f"<h3>빈칸 채우기 ({cnt_t3}문항)</h3>- 빈칸은 `<span class='blank'>&nbsp;&nbsp;&nbsp;&nbsp;</span>` 사용.")
            if select_t4: reqs.append(f'<h3>변형 문장 정오판단 ({cnt_t4}문항)</h3>')
            ...
            if select_t5: reqs.append(f'<h3>객관식: 세부 내용 ({cnt_t5}문항)</h3>{mcq}')
            if select_t6: reqs.append(f'<h3>객관식: 추론 및 비판 ({cnt_t6}문항)</h3>{mcq}')
            if select_t7: reqs.append(f'<h3>객관식: [보기] 적용 ({cnt_t7}문항) [3점]</h3>')
            
            reqs_str = "\n".join(reqs)
            summary_inst = """- **[필수]**: 각 문단 끝에 `<div class='summary-blank'>📝 문단 요약 연습: (이곳에 요약해보세요)</div>` 삽입.""" if use_summary else ""
            
            p1_prompt = f"""
            대한민국 수능 국어 출제 위원장으로서 HTML 문제지를 생성하시오. h1, h2 태그 금지.
            {f"지문 작성 - 주제: {topic}, 난이도: {difficulty} {summary_inst}" if current_d_mode == 'AI 생성' else ""}
            ...
            # [복구] 해설 분할 생성 (Batch) 로직
            total_q_cnt = sum([1 if select_t1 else 0, cnt_t2, cnt_t3, cnt_t4, cnt_t5, cnt_t6, cnt_t7])
            BATCH_SIZE = 6
            ans_parts = []
            sum_done = False
            ...
            for i in range(0, total_q_cnt, BATCH_SIZE):
                ...
                curr_sum_p = """- **[필수]**: 답변 맨 위에 `<div class="summary-ans-box">`를 열고 문단별 요약 예시 답안 작성.""" if use_summary and not sum_done else ""
                sum_done = True
                
                p_chunk = f"""
                수능 해설 위원장으로서 **{s}~{e}번**의 해설만 HTML로 작성하시오.
                {curr_sum_p}
                작성 포맷: <div class="ans-item"><span class="ans-num">[번호] 정답:...</span>...</div>
                문제 내용: {html_q}
                """
...
# ==========================================
# 📖 2. 문학(소설) 문제 제작 함수 (기존 로직 복구)
# ==========================================
def fiction_app():
    with st.sidebar:
        st.header("🏫 타이틀")
        custom_main_title = st.text_input("메인 타이틀", value="사계국어 모의고사", key="fi_t")
        st.header("1️⃣ 작품 정보")
        work_name = st.text_input("작품명", key="fi_n")
        auth_name = st.text_input("작가명", key="fi_a")
        st.header("2️⃣ 유형 및 개수")
        u_v = st.checkbox("어휘(단답)", value=True, key="fv"); c_v = st.number_input("수", 1, 20, 5) if u_v else 0
        u_e = st.checkbox("서술형(심화)", value=True, key="fe"); c_e = st.number_input("수", 1, 10, 3) if u_e else 0
        u_m = st.checkbox("객관식(일반)", value=True, key="fm"); c_m = st.number_input("수", 1, 10, 3) if u_m else 0
        u_b = st.checkbox("객관식(보기)", value=True, key="fb"); c_b = st.number_input("수", 1, 10, 2) if u_b else 0
        st.caption("3️⃣ 분석 활동")
        u5 = st.checkbox("인물 정리", key="f5"); u6 = st.checkbox("상황 요약", key="f6")
        u7 = st.checkbox("관계도/갈등", key="f7"); u8 = st.checkbox("갈등/심리", key="f8")
    ...
    if st.session_state.generation_requested:
        ...
        try:
            ...
            p_q = f"수능 문학 위원장으로서 작품 '{work_name}' 기반 HTML 문제지 작성(h1,h2 금지).\n본문:\n{text}\n유형:\n{reqs_str}"
...
def poetry_app():
    with st.sidebar:
        st.header("🏫 타이틀")
        custom_main_title = st.text_input("메인 타이틀", value="사계국어 모의고사", key="pt_t")
        st.header("1️⃣ 작품 정보")
        po_name = st.text_input("작품명", key="pt_n")
        po_auth = st.text_input("작가명", key="pt_a")
        st.header("2️⃣ 문항 유형 및 개수 (1~5개)")
        c1 = st.checkbox("작품 개요 문제", value=True); n1 = st.number_input("수", 1, 5, 1, key="pn1") if c1 else 0
        c2 = st.checkbox("시상 전개 문제", value=True); n2 = st.number_input("수", 1, 5, 1, key="pn2") if c2 else 0
        c3 = st.checkbox("시어 의미 문제", value=True); n3 = st.number_input("수", 1, 5, 2, key="pn3") if c3 else 0
        c4 = st.checkbox("표현 특징 문제", value=True); n4 = st.number_input("수", 1, 5, 2, key="pn4") if c4 else 0
        c5 = st.checkbox("종합 감상 문제", value=True); n5 = st.number_input("수", 1, 5, 1, key="pn5") if c5 else 0
        c6 = st.checkbox("킬러 키포인트", value=True); n6 = st.number_input("수", 1, 5, 1, key="pn6") if c6 else 0
        c7 = st.checkbox("연계 비교 문제", value=True); n7 = st.number_input("수", 1, 5, 1, key="pn7") if c7 else 0
        st.header("3️⃣ 추가 세트")
        c8 = st.checkbox("OX 세트", value=True); n8 = st.number_input("OX 수", 1, 15, 10, key="pn8") if c8 else 0
        c9 = st.checkbox("서술형 세트", value=True); n9 = st.number_input("서술 수", 1, 10, 3, key="pn9") if c9 else 0
    ...
    if st.session_state.generation_requested:
        ...
        status = st.empty()
        try:
            reqs = [f"문항1. 작품개요 {n1}개", f"문항2. 시상전개 {n2}개", f"문항3. 시어의미 {n3}개", f"문항4. 표현특징 {n4}개", f"문항5. 종합감상 {n5}개", f"문항6. 킬러개념 {n6}개", f"문항7. 연계비교 {n7}개", f"문항8. OX세트 {n8}개(정답표기 금지)", f"문항9. 서술형 {n9}개"]
            reqs_str = "\n".join([r for r in reqs if not r.endswith("0개")])
            
            p_q = f"수능 출제 위원으로서 현대시 '{po_name}' 기반 HTML 학생용 문제지 작성(h1,h2 금지).\n시 본문:\n{po_text}\n요청유형:\n{reqs_str}"
            res_q = generate_content_with_fallback(p_q, status_placeholder=status)
            html_q = res_q.text.replace("```html","").replace("```","").strip()
            
            p_a = f"위 문항들의 정답/해설을 <div class='answer-sheet'>에 작성.\n문제 내용:\n{html_q}"
