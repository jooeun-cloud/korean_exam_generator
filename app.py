...
def non_fiction_app():
    ...
    with st.sidebar:
        ...
        s1 = st.checkbox(label_t1, value=True, key="s1")
        s2 = st.checkbox("2. 내용 일치 O/X", key="s2"); c2 = st.number_input("OX수", 1, 10, 2, key="c2") if s2 else 0
        s3 = st.checkbox("3. 빈칸 채우기", key="s3"); c3 = st.number_input("빈칸수", 1, 10, 2, key="c3") if s3 else 0
        s4 = st.checkbox("4. 변형 문장 정오판단", key="s4"); c4 = st.number_input("판단수", 1, 10, 2, key="c4") if s4 else 0
        s5 = st.checkbox("5. 객관식 (일치)", value=True, key="s5"); c5 = st.number_input("일치수", 1, 10, 2, key="c5") if s5 else 0
        s6 = st.checkbox("6. 객관식 (추론)", value=True, key="s6"); c6 = st.number_input("추론수", 1, 10, 2, key="c6") if s6 else 0
        s7 = st.checkbox("7. 객관식 (보기 3점)", value=True, key="s7"); c7 = st.number_input("보기수", 1, 10, 1, key="c7") if s7 else 0
        use_summary = st.checkbox("📌 문단별 요약 훈련 칸 생성", value=True, key="u_sum")
    ...
    if st.session_state.generation_requested:
        ...
        if not topic and current_d_mode == 'AI 생성': st.warning("주제 입력 필수"); st.session_state.generation_requested = False; return
        status = st.empty()
        try:
            reqs = []
            if s1: reqs.append(f'<div class="question-box"><span class="question-text">1. {label_t1}</span><div class="write-box"></div></div>')
            if s2: reqs.append(f'<h3>내용 일치 O/X ({c2}문항)</h3>')
            ...
            if s5: reqs.append(f'<h3>객관식: 세부 내용 파악 ({c5}문항)</h3>{mcq}')
            if s6: reqs.append(f'<h3>객관식: 추론 및 비판 ({c6}문항)</h3>{mcq}')
            if s7: reqs.append(f'<h3>객관식: [보기] 적용 ({c7}문항) [3점]</h3><div class="example-box">(보기 내용)</div>{mcq}')
            
            reqs_str = "\n".join(reqs)
            summary_inst = "- **[필수]**: 각 문단 끝에 `<div class='summary-blank'>📝 문단 요약 연습: (요약)</div>` 삽입." if use_summary else ""
            
            # [원본 보존] 킬러 문항 가이드라인 포함 프롬프트
            p1_prompt = f"""당신은 대한민국 수능 국어 출제 위원장입니다. 
            # 🚨 h1, h2 사용 금지. 제목 노출 금지.
            {f"**[Step 1] 지문 작성** - 주제: {topic}, 난이도: {difficulty} {summary_inst}" if current_d_mode == 'AI 생성' else ""}
            {f"[사용자 입력 지문 시작]\\n{manual_p}\\n[사용자 입력 지문 끝]" if current_d_mode == '직접 입력' else ""}
            🚨 [고난도 가이드] 1.정보 재구성(1문단+3문단 결합) 2.단어 바꿔치기(Paraphrasing) 3.인과관계 비틀기 4.선지 균형 분포
            **[Step 2] 문제 출제**\\n{reqs_str}"""
            ...
            # [원본 보존] 해설 분할 생성 (Batch Size 6) 로직
            total_q_cnt = sum([1 if s1 else 0, c2, c3, c4, c5, c6, c7])
            BATCH_SIZE = 6; ans_parts = []; sum_done = False
            for i in range(0, total_q_cnt, BATCH_SIZE):
                s, e = i + 1, min(i + BATCH_SIZE, total_q_cnt)
                status.info(f"📝 정답 생성 중... ({s}~{e} / {total_q_cnt})")
                curr_sum_p = "- **[필수]**: 답변 맨 위에 `<div class='summary-ans-box'>`를 열고 문단 요약 답안 작성." if use_summary and not sum_done else ""
                sum_done = True
                p_chunk = f"수능 해설 위원장으로서 {s}~{e}번 해설만 HTML로 작성하시오.\\n{curr_sum_p}\\n문제내용:{html_q}"
...
# ==========================================
# 📖 2. 문학(소설) 문제 제작 함수 (원본 100% 유지)
# ==========================================
def fiction_app():
    with st.sidebar:
        st.header("🏫 타이틀"); custom_main_title = st.text_input("메인 타이틀", value="사계국어 모의고사", key="fic_t")
        ...
        u1 = st.checkbox("1. 어휘 (단답)", value=True); c1 = st.number_input("어휘수", 1, 20, 5) if u1 else 0
        u2 = st.checkbox("2. 서술형 (심화)", value=True); c2 = st.number_input("서술수", 1, 10, 3) if u2 else 0
        u3 = st.checkbox("3. 객관식 (일반)", value=True); c3 = st.number_input("일반수", 1, 10, 3) if u3 else 0
        u4 = st.checkbox("4. 객관식 (보기)", value=True); c4 = st.number_input("보기수", 1, 10, 2) if u4 else 0
        st.caption("3️⃣ 분석 활동"); u5 = st.checkbox("5. 인물 정리"); u6 = st.checkbox("6. 상황 요약"); u7 = st.checkbox("7. 관계도"); u8 = st.checkbox("8. 갈등 정리")
    ...
    if st.session_state.generation_requested:
        ...
        try:
            req_q = [f"어휘 {c1}개", f"서술형 {c2}개", f"객관식 {c3}개", f"보기적용 {c4}개", "인물표" if u5 else "", "상황요약" if u6 else "", "관계도박스" if u7 else "", "갈등정리" if u8 else ""]
            # f-string 에러 방지를 위해 변수 처리
            req_str = "\n".join([r for r in req_q if r])
            p1 = f"수능 위원으로서 작품 '{work_name}' 기반 HTML 문제지 작성(h1,h2 금지).\\n본문: {text}\\n유형:\\n{req_str}"
...
# ==========================================
# 🌸 3. 현대시 문제 제작 함수 (신규 문항화 모드)
# ==========================================
def poetry_app():
    with st.sidebar:
        st.header("🏫 타이틀"); custom_main_title = st.text_input("메인 타이틀", value="사계국어 모의고사", key="po_t")
        st.header("1️⃣ 작품 정보"); po_name = st.text_input("작품명", key="po_n"); po_auth = st.text_input("작가명", key="po_a")
        st.header("2️⃣ 문항 조절 (유형당 1~5개)")
        ...
        t6 = st.checkbox("6. 킬러 키포인트", value=True); n6 = st.number_input("수", 1, 5, 1, key="pn6") if t6 else 0
    ...
    if st.session_state.generation_requested:
        ...
        try:
            reqs = [f"유형1.개요문제 {n1}개", f"유형2.시상전개 {n2}개", f"유형3.시어의미 {n3}개", f"유형4.표현특징 {n4}개", f"유형5.종합감상 {n5}개", f"유형6.킬러포인트 {n6}개", f"유형7.연계비교 {n7}개", f"유형8.OX세트 {n8}개(정답비공개)", f"유형9.서술형 {n9}개"]
            # f-string 백슬래시 에러 방지를 위해 join을 변수로 뺌
            reqs_str = "\n".join([r for r in reqs if not r.endswith("0개")])
            
            p_q = f"수능 국어 위원장으로서 현대시 '{po_name}' 기반 학생용 HTML 시험지 제작(h1,h2 금지).\\n본문: {po_text}\\n[지침]: 절대 정답 표기 금지. 질문 형태로만 구성.\\n[요청]:\\n{reqs_str}"
            res_q = generate_content_with_fallback(p_q, status_placeholder=status)
            html_q = res_q.text.replace("```html","").replace("```","").strip()
            
            p_a = f"위 문항들에 대한 완벽 정답/해설을 <div class='answer-sheet'>에 작성하시오.\\n문제내용: {html_q}"
