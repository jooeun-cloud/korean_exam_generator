...
<head>
    ...
    <style>
        ...
        .ans-main-title {
            ...
            border-bottom: 3px double #333; color: #000;
        }
        .ans-item { 
            margin-bottom: 40px; 
        ...
        .summary-ans-title {
            font-weight: bold; color: #0d6efd; font-size: 1.1em; margin-bottom: 10px; display: block;
        }
        ...
        if current_d_mode == 'AI 생성' and not current_topic:
            ...
            try:
                ...
                **[전체 출력 형식]**
                ...
                - **중요**: 문단 요약 빈칸은 생성하되, 정답은 여기에 적지 마시오.
                ...
                **[Step 2] 문제 출제**
                ...
                if use_summary:
                    if current_d_mode == '직접 입력':
                        ...
                        p_count_info = ""
                        if current_mode == '단일 지문':
                            paras = [p for p in current_manual_passage.split('\n\n') if p.strip()]
                            p_count_info = f"사용자 입력 지문은 총 {len(paras)}개의 문단으로 구성되어 있습니다. 1문단부터 {len(paras)}문단까지 빠짐없이 요약을 작성하시오."
                        else:
                            p_a = st.session_state.manual_passage_input_a.split('\n\n')
                            p_b = st.session_state.manual_passage_input_b.split('\n\n')
                            cnt_a = len([p for p in p_a if p.strip()])
                            cnt_b = len([p for p in p_b if p.strip()])
                            p_count_info = f"(가) 지문은 {cnt_a}개 문단, (나) 지문은 {cnt_b}개 문단입니다. 각각 구분하여 요약을 작성하시오."
                        
                        summary_inst_answer = f"""
                        - **[필수 - 최우선 작성]**: 정답 및 해설 섹션(`answer-sheet`)을 시작하자마자 **가장 먼저** `<div class="summary-ans-box">` 태그를 열고 그 안에 **[문단별 요약 예시 답안]**을 작성하시오.
                        - **[중요]**: {p_count_info} **반드시 사용자 입력 지문의 문단 구분(엔터 두 번)을 기준으로 요약 답안을 생성하시오.**
                        """
                        extra_passage_context = f"\n**[참고: 사용자 입력 지문 원문(문단 구분 중요)]**\n{current_manual_passage}\n"
                    else:
                        summary_inst_answer = """
                        - **[필수 - 최우선 작성]**: 정답 및 해설 섹션(`answer-sheet`)을 시작하자마자 **가장 먼저** `<div class="summary-ans-box">` 태그를 열고 그 안에 **[문단별 요약 예시 답안]**을 작성하시오. 각 문단의 핵심 내용을 1줄씩 요약하여 리스트 형태로 제시하시오.
                        """
                ...
                **[지시사항]**
                ...
                <div class="answer-sheet">
                    <h2 class='ans-main-title'>정답 및 해설</h2>
                    
                    <!-- 문단 요약 옵션 선택 시 여기에 요약 정답 박스 위치 -->
                    <div class="summary-ans-box">
                        <span class="summary-ans-title">[문단별 요약 예시 답안]</span>
                        1문단: 중력가속기의 정의와 초기 형태<br>
                        2문단: 현대 중력가속기의 원리와 정밀도<br>
                        ...
                    </div>
