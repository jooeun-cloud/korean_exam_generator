    ...
    if st.session_state.generation_requested:
        ...
        if current_d_mode == 'AI 생성' and current_mode == "단일 지문 (기본)" and not current_topic:
            ...
            try:
                ...
                난이도: {current_difficulty} (논리적 완결성 및 최상위권 변별력 필수)
                
                **[출력 형식: HTML <body> 내부 태그만 작성 (Markdown 코드블록 ```html 사용 금지, 순수 텍스트로 출력)]**
                
                **1. [절대 금지 사항]:**
                - <html>, <head>, <body> 태그를 절대 쓰지 마시오.
                - 제목(h1), 시간 박스, 지문 본문은 출력하지 마시오. (문제 섹션부터 시작)
                
                **2. [지문 정보 확인]:**
                {passage_instruction}
                {summary_passage_inst}
                
                **3. [문제 출제 및 정답 매핑 규칙 (매우 중요)]:**
                - 문제를 생성할 때, **반드시 내부적으로 정답과 해설을 동시에 확정**하고 기억하십시오.
                - **문제의 순서(1, 2, 3...)와 정답지의 순서(1, 2, 3...)는 100% 일치해야 합니다.**
                - 1번 문제가 '내용 일치'라면, 정답지의 1번 해설도 반드시 '내용 일치'에 대한 것이어야 합니다.
                
                **4. 문제 출제 섹션:**
                {reqs_content}
                
                [HTML 레이아웃 규칙 (Gemma 준수사항)]
                - 전체를 감싸는 컨테이너 없이, 각 유형별로 **<div class="type-box">**를 생성하시오.
                - 유형 제목은 <h3> 태그 사용.
                - **개별 문제는 반드시 <div class="question-box">로 감쌀 것.**
                - **발문(질문):** <b> 태그로 굵게 처리하고, 끝에 <br><br>을 넣어 선지와 간격 확보.
                - **선지:** <div class="choices"> 내부에, 각 선지를 <div>① ...</div> 형태로 작성 ( <br> 태그 사용 금지).
                - **보기/지문:** <div class="write-box"> 또는 <div class="example-box"> 사용.
                - 빈칸은 <span class='blank'></span> 사용.

                ---
                
                **5. [정답 및 해설 섹션] (가장 중요)**
                - 모든 문제가 끝난 후, 문서 맨 마지막에 **<div class="answer-sheet">** 태그를 딱 한 번만 여시오.
                - **[정합성 검증]**: 반드시 위에서 출제한 **문제 번호 순서대로** 해설을 작성하시오.
                - **[형식 지정]**: 각 해설은 아래 형식을 엄격히 따르시오.
                  
                  **[문제번호] 정답 ④**
                  **[해설]** 정답이 되는 이유를 지문의 내용에 근거하여 논리적으로 서술.
                  **[오답 분석]** 오답 선지들이 왜 답이 아닌지 간략히 설명.
                  <br><br>
                
                - **주의:** 문제에 존재하지 않는 번호의 해설을 만들지 마시오. (예: 문제가 3번까지면 해설도 3번까지만 작성)
                - </div> 태그로 answer-sheet를 닫으시오.
                """  
