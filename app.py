        ...
        if current_d_mode == 'AI 생성' and current_mode == "단일 지문 (기본)" and not current_topic:
            ...
            try:
                ...
                **1. [최중요 지시]: 제목(h1, h2), 시간 박스(<div class="time-box">), 그리고 지문 본문은** **절대로 출력하지 마시오.** **출력은 3. 문제 출제 섹션부터 시작하시오.**
                ...
                4. **유형 5, 6, 7 (객관식):**
                    - 정답은 **정답 번호**로 표기할 것.
                    - **[최중요] 해설 시, 선지의 내용을 그대로 복사하여 반복하지 마십시오.**
                    - 각 선지에 대해 **선지 내용 반복 없이** 다음 사항을 간결하게 설명할 것:
                        - **정답 선지:** 해당 선지가 정답인 **결정적 이유**만 간결하게 제시.
                        - **오답 선지:** 해당 선지가 오답인 **틀린 이유** 또는 **지문과의 불일치 근거**만 간결하게 제시.
                    - **[핵심]** 반드시 **모든 선지의 정오(正誤) 판단 근거**를 명시할 것.
                    - **[추가 지시]** 각 선지의 해설이 끝날 때마다 **`<br>` 태그를 사용하여** 줄바꿈을 하여 해설 간 간격을 명확하게 구분할 것.
                """
                prompt_answer_ox = ""
